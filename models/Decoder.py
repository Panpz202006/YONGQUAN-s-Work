...
class CAF(nn.Module):
    ...
    def forward(self,saliency,details,p1,p2):
        ...
        saliency_v=saliency_q.view(B,C,H*W)
        details_v=details.view(B,C,H*W)
        saliency_q=saliency_q.view(B,C,H*W)
        details_k=details_k.view(B,C,H*W)
        attn=saliency_q@details_k.transpose(1,2)*((H*W)**-2)
        attn_score=self.soft(attn.view(B,C*C)).view(B,C,C)
        attn_s=attn_score@saliency_v+saliency_v
        attn_d=attn_score@details_v+details_v
        x=attn_s+attn_d
        x=x.view(B,C,H,W)
        x=self.local(x)
        return x
